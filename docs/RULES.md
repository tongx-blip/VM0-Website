...
| M5 | **`height` on a grid container is not a ceiling.** An implicit row track is `auto`, so the tallest item pushes the frame past the height you declared. Say `grid-template-rows:minmax(0,1fr)`, and `min-height:0` on the children. | Seven tabs came out 545 / 552 / 574: the section jumped as you moved along the strip. | QA §4q12 |
